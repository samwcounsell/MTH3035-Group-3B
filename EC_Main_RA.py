...
df = pd.read_csv("september_trend_ra_mod.csv")

df = pd.read_csv("stra_mod.csv")
...
fig126.update_layout(
    title="Emergent constraint graph comparing 5 year rolling average September SIE trends to predicted Ice Free Year",
    xaxis_title="Year Ice Free Prediction",
    yaxis_title="September SIE Trend (1984 - 2014)",
    font=dict(size=18))
fig126.add_annotation(text="R\N{SUPERSCRIPT TWO} = 0.63",
                  xref="paper", yref="paper",
                  x=0.1, y=0.9, showarrow=False)
#fig126.show()

fig245 = px.scatter(df245, x="ssp245", y="Trend", trendline="ols")
fig245.update_layout(
    title="Emergent constraint graph comparing 5 year rolling average September SIE trends to predicted Ice Free Year",
    xaxis_title="Year Ice Free Prediction",
    yaxis_title="September SIE Trend (1984 - 2014)",
    font=dict(size=18))
fig245.add_annotation(text="R\N{SUPERSCRIPT TWO} = 0.61",
                  xref="paper", yref="paper",
                  x=0.1, y=0.9, showarrow=False)
#fig245.show()
...
fig585.update_layout(
    ...
    yaxis_title="September SIE Trend (1984 - 2014)",
    font=dict(size=18))
fig585.add_annotation(text="R\N{SUPERSCRIPT TWO} = 0.74",
                  xref="paper", yref="paper",
                  x=0.1, y=0.9, showarrow=False)
#fig585.show()
